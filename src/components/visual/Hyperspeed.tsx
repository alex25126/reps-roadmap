...
const Hyperspeed = ({ effectOptions }: { effectOptions: Options }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      assets!: any;
      disposed = false;
      options: Options & { distortion: any };
      road!: Road;
      ...
      constructor(container: HTMLElement, options: Options & { distortion: any }) {
    ...
    class CarLights {
      webgl: App; options: Options & { distortion: any }; colors: any; speed: [number, number]; fade: THREE.Vector2; mesh!: THREE.Mesh;
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          laneX += carShiftX;
          let offsetY = random(options.carFloorSeparation) + (radius as number) * 1.3;
          let offsetZ = -random(options.length);
          aOffset.push(laneX - carWidth / 2, offsetY, offsetZ);
          aOffset.push(laneX + carWidth / 2, offsetY, offsetZ);
          aMetrics.push(radius as number, length as number, speed);
          aMetrics.push(radius as number, length as number, speed);
      ...
      update(time: number) { (this.mesh.material as any).uniforms.uTime.value = time; }
    }
    ...
    class LightsSticks {
      webgl: App; options: Options & { distortion: any }; mesh!: THREE.Mesh;
      ...
      init() {
        ...
        let colors = options.colors.sticks as any;
        if (Array.isArray(colors)) { colors = colors.map((c: number) => new THREE.Color(c)); } else { colors = new THREE.Color(colors); }
    ...
    class Road {
      webgl: App; options: Options & { distortion: any }; uTime = { value: 0 };
  ...
  return <div id="lights" className="w-full h-full" />;
};
